...
engine::perspective_camera::perspective_camera( 
    ...
    m_far_plane(far_z) 
{ 
    m_position = glm::vec3(0.0f, 19.0f, 0.0f);  
    ...
    LOG_CORE_TRACE("3d cam rotation: [{},{},{}]", m_front_vector.x, m_front_vector.y, m_front_vector.z);
}

void engine::perspective_camera::on_update(const timestep& timestep, heightmap terrain)
{
    ...
    else if (engine::input::key_pressed(engine::key_codes::KEY_LEFT_SHIFT)) // Down
        move(e_direction::down, timestep);

    m_position.x = glm::clamp(m_position.x, -19.8f, 19.8f);
    m_position.y = glm::clamp(m_position.y, terrain.ground_height(m_position) + 0.1f , 20.25f);
    m_position.z = glm::clamp(m_position.z, -19.8f, 19.8f);
