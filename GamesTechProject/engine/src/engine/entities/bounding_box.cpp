...
bool engine::bounding_box::collisionPoint(glm::vec3 point) {
	float width1, height1, depth1;
	glm::vec3 bottom1, centre1;
	get(bottom1, width1, height1, depth1);
	centre1 = bottom1 + glm::vec3(width1 / 2.f, height1 / 2.0f, depth1 / 2.f);
	glm::vec3 v = point - centre1;
	if (fabs(v.x) * 2 <= (width1) &&
		fabs(v.y) * 2 <= (height1) &&
		fabs(v.z) * 2 <= (depth1))
		return true; // Collision detected
	else
